...
dataset1 <- as.matrix(read.table("string_interactions.txt", header=TRUE), cols=2)
annotation1 <- as.matrix(read.table("string_interactions_groups_comma_duplicate.txt", header = F))
nrowdat <- nrow(dataset1)
nrowannot <- nrow(annotation1)

fileConn <- file("output2.html", "a")
...
for (i in 1:length(unique(members_with_NA_groups$id))){
...
for(i in 1:(nrowannot-1)){
  genes <- strsplit(annotation1[i, 2], ",")$V2
...
genes <- strsplit(annotation1[nrowannot, 2], ",")$V2
